...
function getListReqProcess(flag,page)
{
	...
	ajax_getListReqProcess(page,select.name,sort_value);
	
	ajax_getPageReqProcess(page,1);
}

//tinh so page
function ajax_getPageReqProcess(page, flag)
{
var xhr = createXHR();
...
//get danh sach req
function ajax_getListReqProcess(page, sort_name, sort_value)
{
  ...
  xhr.onreadystatechange = function(){
      if (xhr.readyState == 4){
          if ((xhr.status  >= 200  &&  xhr.status  <  300) || xhr.status == 304){
              ...
              draw_table_danhsach_reqproccess(list_report, list_report.length,0);
...
function draw_table_danhsach_reqproccess(list_report,length,index)
{
  ...
  for (i = index; i<length; i++)
  {
      ...
      td = tr.insertCell(tr.cells.length);
      td.innerHTML = list_report[i].startDate;
            
      td = tr.insertCell(tr.cells.length);
      td.innerHTML = list_report[i].endDate;
...
function getListReqByPageProccess(page)
{
